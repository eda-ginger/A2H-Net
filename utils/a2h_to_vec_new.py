...
def read_a2h(file_path: str, timesteps: int = 10, all_atom: bool = False, pad: bool = True) -> Data:
    ...
    Args:
        ...
        timesteps (int): Number of timesteps to process
    ...
    for ap, ho in zip(apo, holo):
        ...
        a_atom_name, a_res_name, a_coord = (
            ap['atom_name'], ap['res_name'], ap[['x', 'y', 'z']]
        )

        h_atom_name, h_res_name, h_coord = (
            ho['atom_name'], ho['res_name'], ho[['x', 'y', 'z']]
        )
           
        # Validate atom correspondence
        if (a_atom_name, a_res_name) != (h_atom_name, h_res_name):
            logger.warning(
                f"Atom mismatch detected:\n"
                f"Apo: {a_atom_name} {a_res_name}\n"
                f"Holo: {h_atom_name} {h_res_name}"
            )
            continue
