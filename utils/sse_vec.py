...
class MyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.pkt_window is not None and self.pkt_stride is not None:
            ...
            pkt_tensor = np.zeros((self.max_pkt_len, PT_FEATURE_SIZE)) # 63, 40
            pkt_tensor[:len(_pkt_tensor)] = _pkt_tensor
...
import pandas as pd
import numpy as np
df = pd.read_csv(r"C:\Users\PARK\Desktop\GitHub\A2H-Net\data\preprocessed\sse\pocket\1a0q.csv", index_col=0).drop(['idx'], axis=1)
pkt_tensor =np.zeros((63, 40))
pkt_tensor[:len(df)] = df.values
pkt_tensor.shape
pkt_tensor.astype(np.float32)
...
def global_enconding(seq_path, max_seq_len):
    df = pd.read_csv(seq_path, index_col=0).drop(['idx'], axis=1).values
    seq_tensor = np.zeros((max_seq_len, 40))
    seq_tensor[:len(df)] = df
    return Data(x=torch.from_numpy(seq_tensor.astype(np.float32)))

def pocket_encoding(pkt_path, max_pkt_len):
    df = pd.read_csv(pkt_path, index_col=0).drop(['idx'], axis=1).values
    pkt_tensor = np.zeros((max_pkt_len, 40))
    pkt_tensor[:len(df)] = df
    return Data(x=torch.from_numpy(pkt_tensor.astype(np.float32)))
