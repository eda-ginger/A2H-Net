...
class PrepareData:
    def __init__(self, args):
        ...
        if args.model in ['DeepDTAF', 'CAPLA']:
            self.cache_path = Path(args.cache_dir) / 'Base' / 'Pocket'
        else:
            self.cache_path = Path(args.cache_dir) / 'Base' / 'Sequence'
    ...
    def _check_cache(self):
        ...
        if not (self.cache_path / f"Base_trn.pkl").exists():
            raise FileNotFoundError(f"{self.cache_path / f'Base_trn.pkl'} not found. Please run with force_reload=True.")
        if not (self.cache_path / f"Base_val.pkl").exists():
            raise FileNotFoundError(f"{self.cache_path / f'Base_val.pkl'} not found. Please run with force_reload=True.")
        if not (self.cache_path / f"Base_core.pkl").exists():
            raise FileNotFoundError(f"{self.cache_path / f'Base_core.pkl'} not found. Please run with force_reload=True.")
        if not (self.cache_path / f"Base_tst105.pkl").exists():
            raise FileNotFoundError(f"{self.cache_path / f'Base_tst105.pkl'} not found. Please run with force_reload=True.")
        # if not (self.cache_path / f"Base_tst71.pkl").exists():
        #     raise FileNotFoundError(f"{self.cache_path / f'Base_tst71.pkl'} not found. Please run with force_reload=True.")
        logger.info("All cache files exist.")

    def _make_cache(self):
        # Prepare train and val data for this fold
        trn_df = self.info[self.info['Set'] == 'TRN']
        val_df = self.info[self.info['Set'] == 'VAL']
        ...
        with open(self.cache_path / f"Base_trn.pkl", 'wb') as f:
            pickle.dump(trn, f)
        with open(self.cache_path / f"Base_val.pkl", 'wb') as f:
            pickle.dump(val, f)
                
        # Prepare and save test sets (CORE, tst105, tst71)
        core_df = self.info[self.info['Set'] == 'TST']
        ...
        with open(self.cache_path / f"Base_core.pkl", 'wb') as f:
            pickle.dump(core, f)
        logger.info(f"CORE: {len(core)}")
        
        with open(self.cache_path / f"Base_tst105.pkl", 'wb') as f:
            pickle.dump(tst105, f)
        logger.info(f"TST105: {len(tst105)}")
        # with open(self.cache_path / f"Base_tst71.pkl", 'wb') as f:
        #     pickle.dump(tst71, f)
...
if __name__ == "__main__":
    ...
    for model in ['DeepDTA', 'DeepDTAF']:
        args.model = model
        folder = 'Sequence' if args.model == 'DeepDTA' else 'Pocket'
        ...
        for reload in [True, False]:
            ...
            with open(f'cache/Base/{folder}/Base_trn.pkl', 'rb') as f:
                data = pickle.load(f)
